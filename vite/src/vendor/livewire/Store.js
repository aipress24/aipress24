import EventAction from "@/vendor/livewire/action/event";
import HookManager from "@/vendor/livewire/HookManager";
...
const store = {
  ...
  componentsListeningForEventThatAreTreeAncestors(el, event) {
    ...
    return this.components().filter((component) => {
      return (
        component.listeners.includes(event) && parentIds.includes(component.id)
      );
    });
  },
