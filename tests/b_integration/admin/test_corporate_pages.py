...
"""Integration tests for the admin corporate-pages (mini-CMS) surface.
...
def test_list_page_renders(admin_client: FlaskClient) -> None:
    resp = admin_client.get("/admin/corporate-pages")
...
def test_edit_get_shows_textarea(admin_client: FlaskClient) -> None:
    resp = admin_client.get("/admin/corporate-pages/CGV-BusinessWall/edit")
    assert resp.status_code == 200
    body = resp.data.decode()
