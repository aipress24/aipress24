...
def cms_preview() -> str:
    ...
    Uses the same `render_cms_html` as the public served page so the
    preview can never show something safer than what ships (audit S2).
    """
    body_md = request.form.get("body_md", "") or request.json.get("body_md", "")  # type: ignore[union-attr]
    return render_cms_html(body_md)
