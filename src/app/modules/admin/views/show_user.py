...
@blueprint.route("/show_user/<uid>", methods=["POST"])
@nav(hidden=True)
def show_user_post(uid: str):
    """Handle show user POST actions."""
    user = get_obj(uid, User)
    action = request.form.get("action", "")

    match action:
        case "deactivate":
            _deactivate_profile(user)
            response = Response("")
            response.headers["HX-Redirect"] = url_for("admin.users")
        case "remove_org":
            _remove_organisation(user)
            response = Response("")
            response.headers["HX-Redirect"] = url_for("admin.show_user", uid=uid)
        case "toggle-manager":
            _toggle_manager(user)
            response = Response("")
        case "toggle-leader":
            _toggle_leader(user)
            response = Response("")
...
def _deactivate_profile(user: User) -> None:
...
def _remove_organisation(user: User) -> None:
...
def _toggle_manager(user: User) -> None:
...
def _toggle_leader(user: User) -> None:
