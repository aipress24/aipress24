...
from ..fields import BooleanField, StringField
from ..form import BaseForm


class Form1(BaseForm):
    # Group: competences
    concevoir_et_gerer_la_ligne_editoriale_du_media = BooleanField(
        "Concevoir et gérer la ligne éditoriale du média",
    )
    rediger_des_articles = BooleanField(
        "Rédiger des articles",
    )
    reportage_photo = BooleanField(
        "Reportage photo",
    )
    reportage_audio = BooleanField(
        "Reportage audio",
    )
    reportage_video = BooleanField(
        "Reportage vidéo",
    )
    montage = BooleanField(
        "Montage",
    )
    illustration = BooleanField(
        "Illustration",
    )
    animer_une_table_ronde_conference = BooleanField(
        "Animer une table ronde / conférence",
    )
    media_coaching = BooleanField(
        "Média Coaching",
    )
    enseignement = BooleanField(
        "Enseignement",
    )
    utiliser_des_ia_generatives = BooleanField(
        "Utiliser des IA génératives",
    )
    data_science = BooleanField(
        "Data Science",
    )
    dataviz = BooleanField(
        "DataViz",
    )
    gestion_de_projet_web = BooleanField(
        "Gestion de projet Web",
    )
    gestion_de_projet_d_innovation = BooleanField(
        "Gestion de projet d’innovation",
    )
    gestion_de_plateau_audiovisuel = BooleanField(
...
class Step5J(Step):
    ...
    form_class = Form1
    next_step_id = "step6-j"
    prev_step_id = "step4-j"
